...
- **Status:** ✅ v1.5.0 RELEASED — opt-in hourly check-ins (/checkin + เช็คอิน, window default 08:00–20:00, goal-met skip, on minutely tick), /dnd alias + DND matrix fixed (summary + weekly review now honor per-user DND; ops sends + announcements exempt), zero-LLM pre-parser for NUMBER+UNIT logs (byte-identical confirmations, outage-proof, unit-collision fall-through), health probe 300s + probe_on_startup gate, release announcements (core/release_notes.py catalog, once per user per version, approve catch-up). All 24 ACs PASS; suite 2607/0/1/1xfail; integration Vera PASS. Deployed via bounce; v1.5.0 self-announced. Next: v1.6.0 custom habits (queued). Garmin: off until `[garmin] csv_path`. v1.4.0 deployed (1599/0/1). Garmin: off until `[garmin] csv_path`.
...
- **Queued: v1.6.0 — per-user custom habits** (user request 2026-08-23): users define their own habits from chat; registry becomes per-user (config catalog as base + user-defined). Deep change: extraction prompts, registry-anchored Thai matchers, targets/reminders/streaks/review/charts/history/check-ins all consume the registry. Spec AFTER v1.5.0 ships (must account for v1.5's pre-parser/units extraction + checkins surfaces).
