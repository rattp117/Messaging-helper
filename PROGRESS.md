...
- **Status:** ✅ v1.4.0 RELEASED — /history [<habit>] [<N>] + ย้อนหลัง: per-user statement of own raw entries (ts/habit/value/raw_message, undone marked, unparsed hidden), public menu (9 cmds), budget machinery extracted to core/render_budget.py (byte-identical, audit_view delegates). All 14 ACs PASS; suite 1599/0/1; Vera verdict ready-to-ship. Deployed via bounce. Garmin: off until `[garmin] csv_path`.
