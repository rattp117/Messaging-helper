...
(ROADMAP.md v0.2.0, AC2.1-AC2.5): the ordered model chain in
OllamaClient.chat_json, the startup schema-conformance probe
...
async def test_probe_reports_conformant_model_true():
    ...
    results = await llm.probe_schema_support()
...
async def test_probe_extra_key_is_non_conformant():
    """AC2.1's exact-keys check: 5 required keys PLUS an extra key must not
    count as conformant."""
    content = json.dumps(
        {
            "category": "water",
...
async def test_probe_missing_key_is_non_conformant():
    content = json.dumps({"category": "water", "water_ml": 500, "confidence": 0.9})  # missing 2 keys
...
async def test_probe_unreachable_model_reports_false_and_does_not_raise():
    ...
    results = await llm.probe_schema_support()  # must not raise
...
async def test_probe_checks_every_configured_model_independently():
    ...
    good_content = json_payload(category="water", water_ml=500, confidence=0.9)
    llm = make_client(
...
async def test_fallback_first_model_off_schema_second_model_wins():
    ...
    valid_content = json_payload(category="water", water_ml=500, confidence=0.9)
    llm = make_client(
...
async def test_fallback_first_model_unparseable_json_second_model_wins():
    ...
    valid_content = json_payload(category="stretch", stretch_min=10, confidence=0.9)
    llm = make_client(
    ...
    assert result == ExtractionResult("stretch", None, 10, None, 0.9)
...
# ---------------------------------------------------------------------------
# AC2.3 -- confidence threshold gate, exercised through the handler path
# ---------------------------------------------------------------------------


async def test_below_threshold_confidence_yields_clarifying_question_and_no_row(db):
    """AC2.3: a schema-valid water extraction whose confidence (0.3) is
    below the configured threshold (0.55) must clarify, not log."""
    content = json_payload(category="water", water_ml=500, confidence=0.3)
...
async def test_at_threshold_confidence_is_logged(db):
    ...
    content = json_payload(category="water", water_ml=500, confidence=0.55)
    llm = _StaticLLM(content)
    channel = _RecordingChannel()
    config = Config.model_validate({"ollama": {"confidence_threshold": 0.55}})
...
async def test_custom_threshold_from_config_is_honored(db):
    ...
    content = json_payload(category="water", water_ml=500, confidence=0.6)
    llm = _StaticLLM(content)
    channel = _RecordingChannel()
...
async def test_below_threshold_diary_also_clarifies_and_writes_no_row(db):
    """AC2.3 applies uniformly across categories, not just water."""
    content = json_payload(category="diary", diary_text="a quiet day", confidence=0.2)
    llm = _StaticLLM(content)
    channel = _RecordingChannel()
    config = Config.model_validate({"ollama": {"confidence_threshold": 0.55}})
...
async def test_bare_string_model_behaves_identically_to_list_of_one():
    ...
    bare_result = await parse_message("did 10 min stretch", bare_llm, GLASS_ML, BOTTLE_ML)
    list_result = await parse_message("did 10 min stretch", list_llm, GLASS_ML, BOTTLE_ML)

    assert bare_result == list_result == ExtractionResult("stretch", None, 10, None, 0.95)
...
async def test_chat_json_all_models_connect_error_fails_closed():
    ...
    result = await parse_message("500ml", llm, GLASS_ML, BOTTLE_ML)  # must not raise
