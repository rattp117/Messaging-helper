...
async def test_goal_already_met_uses_db_override_not_config_default(db):
    ...
    assert reminders._goal_already_met(db, water, config, OWNER) is False  # under both

    db.set_target(OWNER, "water", 500.0)
    assert reminders._goal_already_met(db, water, config, OWNER) is True  # 1000 >= override 500
...
async def test_goal_already_met_applies_to_previously_goalless_duration_habit(db):
    ...
    assert reminders._goal_already_met(db, stretch, config, OWNER) is False  # no goal at all yet

    db.set_target(OWNER, "stretch", 20.0)
    assert reminders._goal_already_met(db, stretch, config, OWNER) is True  # 20 >= target 20

    db.clear_target(OWNER, "stretch")
    assert reminders._goal_already_met(db, stretch, config, OWNER) is False  # back to "no goal"
...
def test_day_qualifies_and_compute_streak_use_override(db, registry):
    ...
    assert streaks.day_qualifies(db, config, water, "2026-08-19", OWNER) is False  # under config default 2500

    db.set_target(OWNER, "water", 1000.0)
    assert streaks.day_qualifies(db, config, water, "2026-08-19", OWNER) is True  # meets override
    assert streaks.compute_streak(db, config, water, date(2026, 8, 19), OWNER) == 1
...
def test_daily_summary_reflects_override(db, registry):
    ...
    lines = streaks.compute_daily_summary(db, config, registry, date(2026, 8, 19), OWNER)
...
def test_chart_goal_line_reflects_override(db, registry, monkeypatch):
    ...
    charts.render_habit_chart(db, config, water, date(2026, 8, 19), "en", OWNER)
...
async def test_generic_numeric_confirmation_percentage_uses_override(db, monkeypatch):
    ...
    db.set_target(OWNER, "juice", 500.0)
...
def test_override_persists_across_a_fresh_database_instance(tmp_path, registry):
    ...
    first.set_target(OWNER, "water", 1800.0)
    first.close()

    second = Database(db_path)
    try:
        water = registry.get("water")
        assert targets.effective_goal(second, water, config, OWNER) == 1800.0
    finally:
        second.close()
...
def test_compute_streak_reads_get_target_at_most_once_per_call(db, registry):
    ...
    db.set_target(OWNER, "water", 2500.0)

    calls = {"n": 0}
    real_get_target = Database.get_target

    def counting_get_target(self, user_id, habit_id):
        calls["n"] += 1
        return real_get_target(self, user_id, habit_id)
    ...
    try:
        streak = streaks.compute_streak(db, config, water, date(2026, 8, 19), OWNER)
    finally:
