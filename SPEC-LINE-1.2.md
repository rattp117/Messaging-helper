...
- `monthly_push_total` read failure at the quota gate: **fail-closed** (drop the non-owner push), logged loudly — RESOLVED by Archi ruling 2026-08-31 ("the bill can never surprise me"); owner pushes and all replies are unaffected (see §9 OQ3).
...
- **R-Q7 (gate fail-closed)** RESOLVED by Archi ruling 2026-08-31: fail-**closed** — rationale: the user's requirement is "the bill can never surprise me," so a `monthly_push_total` read exception inside the gate is logged **loudly** (ERROR) and treated as "deny" for that one non-owner push (no `_send_push`, no increment, no raise). The owner's own pushes and every reply (R-Q8) are unaffected — the failure is only ever consulted for a non-owner `chat_id`. The subsequent successful increment/read re-engages the cap normally. (Originally specified fail-open below, superseded by this ruling; §9 OQ3 updated to match.)
...
- **OQ3 — Quota-gate failure disposition. RESOLVED by Archi ruling 2026-08-31: fail-closed** — rationale: the user's requirement is "the bill can never surprise me." On a `monthly_push_total` read error, non-owner proactive pushes **stop** (logged loudly); owner pushes and all replies are unaffected. This supersedes the originally-considered fail-open alternative (bounded overspend, described below for context only): fail-open would trade a rare bounded overspend for availability, protecting realtime traffic over money — the ruling weighs the opposite way for this deployment. `R-Q7`/`channels/line.py:_monthly_push_total_fail_closed` implement this directly.
